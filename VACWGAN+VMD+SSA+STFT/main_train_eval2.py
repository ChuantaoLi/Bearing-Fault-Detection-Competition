# ===================================================================================
# 文件名: main_train_eval.py (最终修正 - 自定义样本生成数量)
# ===================================================================================
...
VACWGAN_EPOCHS = 300
VACWGAN_LR = 1e-4
# VACWGAN_SAMPLES_PER_CLASS = 50  # (已移除) 改用下面的自定义字典
LATENT_DIM = 100
...
def load_and_augment_data():
    ...
    # 遍历自定义的生成目标字典
    for class_name, num_to_generate in GENERATION_TARGETS.items():

        # 跳过请求为 0 的类别
        if num_to_generate == 0:
            print(f"跳过类别: {class_name} (要求 0 个样本)")
            continue

        # 获取该类别的整数标签
        label_int = CLEAN_LABEL_MAP.get(class_name)
        if label_int is None:
            print(f"警告: 在 CLEAN_LABEL_MAP 中未找到类别 {class_name}。")
            continue

        print(f"正在为类别 '{class_name}' (标签 {label_int}) 合成 {num_to_generate} 个样本...")

        # 获取该类别的所有真实样本，用于输入 Encoder
        X_minority = X_processed[y_labels == label_int]
        y_minority = y_labels[y_labels == label_int]

        if X_minority.size == 0:
            print(f"警告: 类别 {class_name} 没有原始样本，无法作为生成基础。跳过...")
            continue
        ...
        if X_synth.size > 0:
            ...
            print(f"  > 成功生成 {X_synth.shape[0]} 个样本。")
...
def run_test_inference(optimization_dir, class_names, best_params):
    ...
    output_filename = "test_prediction_results2.txt"
